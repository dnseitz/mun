...
class YelpDataModelTests: YAPIXCTestCase {
  
  func test_ModelCreation_FromEncodedJSON() {
    do {
      let dict = try self.dictFromBase64(ResponseInjections.yelpValidOneBusinessResponse)
      let business = YelpBusiness(withDict: (dict["businesses"] as! [[String: AnyObject]])[0])
      ...
      XCTFail("Failed to parse embedded json response")
    }
  }
